...
> 状态：已采纳 · 版本：v3 · 更新：2026-09-25 · 最后对照代码：2026-09-25（注册审批威胁行与限额、日历限额） · 依据 ADR-0001 §4.6 §9.4、01 §1 §3.8 §3.12 §4、02 §2 §7 §9、05 §8。多用户（邀请 或 注册 + 审批，ADR-0008）下的威胁模型、数据保留、账号生命周期、限额。散在各规范的安全与保留规则以本文集中表为准；本文与其他规范冲突时，先改本文再改代码。需求编号见 `00-requirements.md` 的 REQ-AUTH-* / REQ-WS-* / REQ-ATTACH-* / REQ-OPS-*。
...
| 威胁 | 场景 | 缓解 | 状态 |
|---|---|---|---|
| 自助注册滥用 | 攻击者批量建号 | ~~`disableSignUp`，只经邀请（01 §2）~~ 注（2026-09-25，ADR-0008）：开放注册但**必经审批**——注册须过服务端拼图、每 IP 5 次 / 小时、待审批总量 ≤ 200；未批准账号无 `member` 行，会话视为未登录，登录返回 403 `REGISTRATION_PENDING` 且不下发 Cookie；Better Auth `/sign-up/email` 仍关闭；驳回即删号 | 已覆盖 |
| 注册探测邮箱 / 用户名 | 用注册接口的 409 判断某邮箱是否已注册 | 接受（个人工作台规模，换取明确的字段级提示）；拼图 + IP 限流抬高批量探测成本；登录接口仍不区分「不存在」与「密码错」 | 已接受 |
...
| 注册（ADR-0008） | 每 IP 5 次 / 小时；待审批 ≤ 200；密码 ≥ 8 位；用户名 3–30 位 | 429 `RATE_LIMITED` / 422 | 02 §9 |
| 日历（ADR-0009） | 每人 ≤ 30 个日历；日程区间查询 ≤ 400 天、单次展开 ≤ 2000 次；提醒 ≤ 5 个 | 422 | 02 §9 |
