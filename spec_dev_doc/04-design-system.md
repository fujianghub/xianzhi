...
> 注 2026-09-24（对照代码）：复合组件实际放在 `components/ui/`（未拆 `ui-x/`）。新增：`Seal`（燕印，ADR-0005）、`Disclosure`（展开指示，REQ-UI-030）、`XzMotionConfig`（动效档位，REQ-UI-028）、`SliderCaptcha` 与 `IconField`（登录，ADR-0006）；布局层 `ThemeMenu`（未登录页主题选择）；领域层 `CalendarViews`（月 / 周视图，REQ-UI-031）。侧栏导航与空间树共用 `.xz-nav-item`（REQ-UI-032）。
...
- 图标：Lucide，`stroke-width 1.75`，尺寸 16/20/24。（注 2026-09-26，ADR-0015：记录类型有固定图标映射；色块 `.xz-chip` = 色板浅底 → 实色渐变 + 高光 + 细描边，用于类型 / 大类 / 空间 / 面板标题，色块内图标 stroke 2；单色图标优先用 `--xz-icon-*` 专属色，避免整屏灰；目录树层级 = 引导线 + 字重递减，见 REQ-KB-006。）
