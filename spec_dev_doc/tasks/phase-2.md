...
| T1-014 · T1-015 | 正文 `@` 提及、~~`[[` 双链选择器~~、entryLink 行内 / 标题 / 卡片三形态切换、单个 URL 粘贴转 entryLink | REQ-EDITOR-010 · 011 | 本属 Phase 2；一期斜杠菜单已可插入记录卡片 / 链接；注 2026-09-25：`[[` 已触发记录选择器 |
| T1-014 | mermaid / KaTeX 渲染、`:::kind` 以外的 callout 细节 | REQ-EDITOR-007 · 008 · 009 | 一期为占位节点（源码显示）；`:::kind ` 快捷转换已实现 |
| T1-017 | ~~Aside 反链~~、~~历史（快照浏览 / 恢复）页签~~ | REQ-COLLAB-008 等 | 一期页签为「后续版本提供」；注 2026-09-25：历史页签已实现（ADR-0011 §4）；2026-09-26：反链 / 关联页签已实现（ADR-0012） |
