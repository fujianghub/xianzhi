/**
 * 标签（01 §3.7、02 §9、REQ-TAG-001 · 002 · REQ-TAG-004 ~ 006）：工作区内名字唯一，颜色为 9 色 token 名（ADR-0010）。
 * 权限：创建 = 非 guest（tag.create，TagPicker 输入即创建，可选色）；
 * 改名 / 改色 / 合并 / 删除 = 管理员，或该标签的创建者（tag.manage + TagRef，ADR-0014；迁移前的旧标签无创建者 → 仅管理员）。
 */
...
/** GET /tags：全部标签（数量级几十，不分页），附使用次数（仅计数，不泄露对象）与 canManage。 */
...
export async function deleteTag(db: Db, ctx: TagCtx, id: string): Promise<void> {
  assertCan(ctx.actor, 'tag.manage', refOf(await getTag(db, ctx, id)))
  await db.delete(tags).where(and(eq(tags.id, id), eq(tags.workspaceId, ctx.workspaceId)))
}
