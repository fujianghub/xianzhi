/**
 * 分类目录树（ADR-0012、REQ-KB-005）：`entries.parent_id` + `tree_order`（fractional-indexing，COLLATE "C"）。
 * - `tree_order` 为 null = 不在目录（「其余记录」）；在目录中的记录父页必须同分类且也在目录中。
 * - 一次取整棵树（只取标题等元数据，不取正文，不变量 6）；可见性同列表（visibleEntriesWhere），读不到的节点连同其子树不出现。
 * - 移动：需对被移动记录 entry.write；父页须同分类、可读、在目录中，且不能是自己或自己的后代（递归 CTE 防环）；
 *   `after` 须为新父页下的同级。只改被移动的一行。
 * - 软删父页 / 移到别的分类：其子页上移一级（接到它原来的父页、保持相对顺序在其位置）。
 */
...
async function requireParent(db: DbOrTx, ctx: EntryCtx, spaceId: string, parentId: string) {
  ...
  if (p.row.spaceId !== spaceId)
    throw AppError.validation([{ path: 'parentId', message: '父页须在同一分类' }])
